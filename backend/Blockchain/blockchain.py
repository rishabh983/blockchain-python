...
class Blockchain:
    ...
    def from_json(chain_json):
    	"""
		Deserialize alsit of serialised blocks into a Blockchain instance.
		The result will conatin a chain list of Block instances.
    	"""
                ...
                if transaction.input == MINING_REWARD_INPUT:
                    if has_mining_reward:
                        raise Exception('there can only be one mining reward per block.'\
                            f'Check block wih hash: {block.hash}'
                        )
                    ...
                    historic_blockchain = Wallet.calculate_balance(
                        historic_blockchain, transaction.input['address']
                    )

                    if historic_blockchain != transaction.input['amount']:
                        raise Exception(
